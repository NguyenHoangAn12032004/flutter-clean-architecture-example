...
import 'package:rickmorty/presentation/shared/character_list_item.dart';
import 'package:rickmorty/presentation/shared/character_list_item_header.dart';
import 'package:rickmorty/presentation/shared/character_list_item_loading.dart';
import 'package:rickmorty/presentation/ui/details_page/character_details_page.dart';
import 'package:rickmorty/presentation/controllers/list_page/character_page_bloc.dart';
...
class __ContentState extends State<_Content> {
  ...
  Widget build(BuildContext ctx) {
    ...
    return BlocListener<CharacterPageBloc, CharacterPageState>(
      listener: (context, state) {
        if (state.status == CharacterPageStatus.failure) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Refresh failed. Tap to retry.')),
          );
        }
      },
  ...
  Future<void> _onRefresh() async {
    final bloc = context.read<CharacterPageBloc>();
    final completer = Completer<void>();
    late final StreamSubscription<CharacterPageState> sub;
    sub = bloc.stream.listen((state) {
      if (state.status == CharacterPageStatus.success ||
          state.status == CharacterPageStatus.failure) {
        if (!completer.isCompleted) completer.complete();
        sub.cancel();
      }
    });
    bloc.add(const RefreshCharactersEvent());
